...
PreProcessingData = function(run_name, circascore='all'){
  ...
  path_to_data = paste0('e:/R/AD_proj/',run_name,'_norm_combined.parquet')
...
MetaCycleAnalysis <- function(data, condition, run_name, path_to_save, date, 
  ...
  for (item in items_to_process) {
    ...
    for (ZT in names(datasamplezt)) {
      ...
      cols_for_summary <- if(!is.null(genes_to_analyze)) intersect(genes_to_analyze, names(df)) else names(df)[1:4171]
